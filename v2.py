...
for i, target_name in enumerate(columns):
    ...
    # 计算误差水平分类
    if target_name in ['CO2_density_fast_tmpr', 'CO2_density', 'T_SONIC']:
        error_level = "高"
    else:
        error_level = "中低"
...
# 按误差水平分组统计
print("\n== 按误差水平分组的平均性能 ==")
grouped_performance = performance_df.groupby('误差水平').mean()
print(grouped_performance.to_string(float_format=lambda x: f"{x:.4f}"))

# 特征重要性分析
print("\n== 特征重要性分析 ==")
if hasattr(final_model, 'feature_importances_') or hasattr(final_model.estimators_[0], 'feature_importances_'):
    try:
        # 尝试获取特征重要性
        importances = None
        if hasattr(final_model, 'feature_importances_'):
            importances = final_model.feature_importances_
        elif hasattr(final_model.estimators_[0], 'feature_importances_'):
            # 取第一个目标的模型的特征重要性
        ...
        print("获取特征重要性失败，可能与模型类型有关")
else:
    # 如果找不到特征重要性，尝试使用目标特定模型
    if target_specific_models:
        for target, model in list(target_specific_models.items())[:2]:  # 只显示前两个目标的特征重要性
...
# 保存详细性能报告到CSV
performance_file = "model_performance_report.csv"
performance_df.to_csv(performance_file, index=False)
print(f"\n详细性能报告已保存至 {performance_file}")

# 高误差特征的预测与真实值比较
try:
    print("\n== 高误差特征预测样本 ==")
    sample_size = min(5, len(y_val))
    sample_indices = np.random.choice(len(y_val), sample_size, replace=False)
    
    for target_name in ['CO2_density_fast_tmpr', 'CO2_density', 'T_SONIC']:
        i = columns.index(target_name)
        print(f"\n{target_name} 样本比较 (真实值 vs 预测值):")
        for idx in sample_indices:
            true_val = y_val.iloc[idx][target_name]
            pred_val = y_val_pred[idx, i]
            error = abs(true_val - pred_val)
            error_pct = (error / abs(true_val + 1e-10)) * 100
            print(f"  样本 {idx}: 真实值={true_val:.4f}, 预测值={pred_val:.4f}, 误差={error:.4f} ({error_pct:.2f}%)")
except Exception as e:
    print(f"生成样本比较时出错: {str(e)}")
...
if hours > 0:
    ...
    print(f"总运行时间: {int(minutes)}分钟 {seconds:.2f}秒")
print(f"处理的特征数量: {len(important_features)}")
print(f"训练样本数量: {len(X_train_scaled)}")
print(f"验证样本数量: {len(X_val_scaled)}")
print(f"测试样本数量: {len(X_test_scaled)}")

# 导出最终预测结果时添加时间戳
from datetime import datetime
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
final_result_file = f"result_Enhanced_CatBoost_Stacked_{timestamp}.csv"
result_df.to_csv(final_result_file, index=False)
print(f"\n最终预测结果已保存至 {final_result_file}")
